...
class JSONLoader(threading.Thread):
    ...
    def run(self):
        ...
        ssearch = SiteSearch(self.__host, self.__dns, uloginpage, uname, unamefm, upass, upassfm, forms=False,
                             debug=True, keywords=6, maxpages=10)
        ssearch.start()
        # Wit for completion
...
"""
def dnslookup(name, dns_svr):
    # Do we have both
    if name and dns_svr:
            # Use PyDNS to lookup ip addr
            print("Testing %s with %s" % (name, dns_svr))
            myres = dns.resolver.Resolver()
            myres.nameservers = [dns_svr]
            answer = myres.query(name)
            # Go through the responses
            for a in answer.response.answer: 
                match_ip = ip_re.search(a.to_text())
                if match_ip:
                    return match_ip.groups()[0]
    return None
"""
    arethreaddone
...
def loadfromjson(jsonpath):
    ...
    json.dump(jsondata, jsonout, indent=4)
    jsonout.close()
    print("Done!")
