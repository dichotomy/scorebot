...
def get_json(object_data):
    if object_data:
        if not isinstance(object_data, QuerySet) and not (
                type(object_data) == list and isinstance(object_data[0], models.Model)):
...
def get_object_with_id(request, object_class, object_id=None, object_limit=200, object_page=0, object_filter=None,
    ...
    if object_class is not None:
        ...
        if object_model is not None:
            ...
            if object_id is not None:
                ...
                try:
                    object_data = object_model.filter(**object_filter)
                    return HttpResponse(get_json(object_data)) if object_response else object_data
                except Exception:
                    logger.debug(__name__, 'Requested invalid value "%s" for model "%s"!' % (dict(**object_filter),
                                                                                             object_class.__name__))
                    return HttpResponseBadRequest(
                        content='SBE [API]: Object with "%s" does not exist in the requested data set!' % str(
                            dict(**object_filter))) \
                        if object_response else None
            else:
                try:
...
def get_object_by_query(request, object_data, object_limit=200, object_page=0, object_response=True):
    if object_data is not None:
        if not isinstance(object_data, QuerySet):
            logger.error(__name__, 'Attempted to call "get_object_by_query" on an object that is not a QuerySet!')
            return HttpResponseBadRequest('SBE [API]: Attempted to use an incorrect function for that object type!') \
                if object_response else None
        if not request.authkey['ALL_READ'] and not request.authkey['%s.READ' % object_data.model.__name__]:
            logger.warning(__name__, 'Model "%s" could not be requested from the database due to permissions!'
                           % object_data.model.__name__)
            return HttpResponseForbidden(content='SBE [API]: You do not have permissions to read "%s"!' %
                                                 object_data.model.__name__) if object_response else None
        logger.debug(__name__, 'Model "%s" was read from the database!' % object_data.model.__name__)
        if len(object_data) > 0:
            ...
            return HttpResponseNotFound(content='SBE [API]: There is no data in the requested data set!') \
                if object_response else None
    logger.warning(__name__, 'Requested non-existent data set model!')
    return HttpResponseNotFound(content='SBE [API]: The requested data set does not exist!') \
        if object_response else None


def get_object_by_filter(request, object_class, object_limit=200, object_page=0, object_filter=None,
                       object_response=True):
    if object_class is not None:
        object_model = get_query_set(object_class)
        if object_model is not None:
