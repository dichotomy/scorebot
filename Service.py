...
class Service:
   ...
   def __init__(self, port, protocol, application, value, logger):
      '''
      Constructor
      '''
      self.logger = logger
   ...
   def check(self, this_round, ipaddress, timeout):
      ...
      if tcp_80_re.match(service_name):
         ...
         try:
            if globalvars.verbose:
               self.logger.err("\t\t\tTrying to login...")
            myftp.login("blueteam", "abcd1234")
            if globalvars.verbose:
      ...
      elif udp_53_re.match(service_name):
         #self.set_score(this_round, myscore)
         self.set_score(this_round, 0)
         self.logger.err("NEED TO IMPLEMENT 53/TCP SCORE CHECKING!!\n")
      else:
         ############################################
         try:
            if globalvars.verbose:
   ...
   #sometimes we just max the scores because the host is down
