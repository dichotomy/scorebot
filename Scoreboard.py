...
class Scoreboard(threading.Thread):
    ...
    def calc_scores(self):
        ...
        for team in self.teams.keys():
            scores = self.teams[team].get_all_rounds()
            for round in scores:
                if round not in self.team_scores:
                    self.team_scores[round] = {}
                self.team_scores[round][team] = scores[round]
                #print "round: %s team: %s score: %s" % (round, team, scores[round])
    ...
    def run(self):
        while True:
            try:
                ...
                self.logger.err("Error processing score:\n")
