...
urlpatterns = [
    ...
    url(r'^game/(?P<game_id>[0-9]+)/host/(?P<host_id>[0-9]+)/service/(?P<service_id>[0-9]+)/$', GameViews.game_service,
        name='game_service'),
    url(r'^game/(?P<game_id>[0-9]+)/host/(?P<host_id>[0-9]+)/compromise/$', GameViews.game_compromise,
        name='game_compromise'),
    url(r'^game/(?P<game_id>[0-9]+)/host/(?P<host_id>[0-9]+)/compromise/(?P<compromise_id>[0-9]+)/$',
        GameViews.game_compromise, name='game_compromise'),
    url(r'^game/(?P<game_id>[0-9]+)/host/(?P<host_id>[0-9]+)/service/(?P<service_id>[0-9]+)/content/$',
        GameViews.game_content, name='game_content'),
    url(
        r'^game/(?P<game_id>[0-9]+)/host/(?P<host_id>[0-9]+)/service/(?P<service_id>[0-9]+)/content/(?P<content_id>[0-9]+)/$',
        GameViews.game_content, name='game_content'),
